...
Windows では `faster-whisper==0.10.1` が依存する `av==10.x` に利用可能な wheel がないため、`requirements.txt` では `faster-whisper` を自動インストールしません。データセット作成時の文字起こしは、その場合自動で Hugging Face Whisper にフォールバックします。

Windows で `faster-whisper` を使いたい場合は、通常の依存関係を入れたあとで別途次を実行してください。

```bash
uv pip install "faster-whisper==1.1.1"
```

この場合、書き起こし時には自動で `faster-whisper` が使われます。なお、このリポジトリでは過去に `faster-whisper` 1.0.0 系で品質劣化があったため 0.10.1 に固定していた経緯があるので、Windows では互換性優先で 1.1.1 を案内しています。まずは数本の音声で品質確認をすることをおすすめします。
...
#### CIAIR-VCVをこのリポジトリで自動処理する実運用手順

この節は、CIAIR-VCVのように「元から`wav`と`txt`が対になって存在するコーパス」を、このリポジトリで前処理から学習、サンプル作成まで一気に進めるときの実運用手順です。別スレッドのAIへ前提情報として渡すなら、この節をそのまま共有してください。

前提は以下です。

- Windows環境で、このリポジトリの仮想環境を有効化していること。
- `CIAIR-VCV/...` のように、話者フォルダの下に `001.wav` と `001.txt` のような対応ファイルがあること。
- 書き起こしの正とする情報源はWhisperではなく、元コーパスの `txt` であること。
- CIAIRのタグ付き発音は `scripts/preprocess_ciair_corpus.py` 側で学習用の発話文字列へ平坦化すること。
- `preprocess_text.py` は必ず `--correct_path` 付きで実行すること。

CIAIR向けの運用上の扱いは以下です。

- `txt` は `cp932` で読み込みます。
- 各行は `表記 & 発音` の形式を前提にします。
- `(W xxx)` のようなタグ付き発音は、タグを落として学習用の発話文字列に変換します。
- `<H>` は長音 `ー` に変換し、それ以外の山括弧タグは除去します。
- 発話文字列が空になる行はスキップします。
- 長い無音は自動検出し、必要に応じて分割します。確認が必要なものは `review_needed.tsv` と `silence_report.tsv` に出力します。
- `wav` が存在しない `txt` は使われません。処理対象は `wav` 側を基準に決まります。

実行順は以下です。ここでは例として `CIAIR-VCV/m06/m06008/m0600802` から `m0600802_all` を作る例を書きます。
...
2. CIAIR用の前処理スクリプトでデータセットを作成します。

```powershell
python scripts/preprocess_ciair_corpus.py ^
  --input_dir CIAIR-VCV/m06/m06008/m0600802 ^
  --model_name m0600802_all ^
  --speaker_name m0600802
```

この段階で主に以下が生成されます。

- `Data/m0600802_all/raw/`: 切り出し済みの元音声
- `Data/m0600802_all/esd.list`: 学習用の転記一覧
- `Data/m0600802_all/ciair_manifest.tsv`: 元コーパスとの対応表
- `Data/m0600802_all/review_needed.tsv`: 要確認項目
- `Data/m0600802_all/silence_report.tsv`: 無音検出レポート

3. 必要なら `review_needed.tsv` と `esd.list` を確認します。空行や不自然な切り出しがないかを見る工程です。

4. 学習設定を初期化します。JP-Extra前提で、100 epoch保存の設定にします。

```powershell
python -c "from gradio_tabs.train import initialize; print(initialize('m0600802_all', 2, 100, 1000, False, False, False, False, False, True, 200))"
```

この処理で `Data/m0600802_all/config.json`、`Data/m0600802_all/models/`、`model_assets/m0600802_all/config.json` などの土台が準備されます。

5. 音声を学習用形式へリサンプルします。

```powershell
python resample.py -i Data/m0600802_all/raw -o Data/m0600802_all/wavs --num_processes 6 --sr 44100
```

6. 転記を学習用リストへ変換します。CIAIR系では `--correct_path` が重要です。

```powershell
python preprocess_text.py ^
  --config-path Data/m0600802_all/config.json ^
  --transcription-path Data/m0600802_all/esd.list ^
  --train-path Data/m0600802_all/train.list ^
  --val-path Data/m0600802_all/val.list ^
  --val-per-lang 1 ^
  --yomi_error skip ^
  --correct_path ^
  --use_jp_extra
...
```powershell
python bert_gen.py --config Data/m0600802_all/config.json
```

8. スタイル特徴量を生成します。

```powershell
python style_gen.py --config Data/m0600802_all/config.json --num_processes 6
```

9. 学習を開始します。

```powershell
python train_ms_jp_extra.py --config Data/m0600802_all/config.json --model Data/m0600802_all
```

通常は以下を確認します。

- `Data/m0600802_all/models/` に中間チェックポイントが増えること
- `model_assets/m0600802_all/` に推論用アセットが出ること
- 最終的に `model_assets/m0600802_all/m0600802_all_e100_sXXXX.safetensors` が生成されること

10. 学習後にサンプル音声を自動生成します。

```powershell
python scripts/generate_model_samples.py ^
  --model-dir model_assets/m0600802_all ^
  --model-file m0600802_all_e100_sXXXX.safetensors ^
  --device cuda
```

生成物は以下です。

- `model_assets/m0600802_all/samples_m0600802_all_e100_sXXXX/sample_01.wav`
- `model_assets/m0600802_all/samples_m0600802_all_e100_sXXXX/sample_02.wav`
- `model_assets/m0600802_all/samples_m0600802_all_e100_sXXXX/sample_03.wav`

AIへ依頼するときの最小前提は以下です。

- 対象フォルダは `CIAIR-VCV/.../<speaker>/<session>`
- `wav` と `txt` のペアから `scripts/preprocess_ciair_corpus.py` で `Data/<model_name>` を作る
- 文字起こしの正はWhisperではなく元の `txt`
- `initialize` → `resample.py` → `preprocess_text.py --correct_path --yomi_error skip --use_jp_extra` → `bert_gen.py` → `style_gen.py` → `train_ms_jp_extra.py` → `scripts/generate_model_samples.py` の順で進める
- 学習完了条件は最終 `.safetensors` と3つのサンプルwavの生成

この運用は、`m0600202_all`、`m0600302_all`、`m0600402_all`、`m0600502_all`、`m0600602_all`、`m0600702_all` で使った手順と同じです。
